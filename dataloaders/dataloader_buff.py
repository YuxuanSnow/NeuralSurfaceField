...
class DataLoader_Buff_depth_rootfinding(BaseLoader):
    ...
    def _init_dataset(self):
        ...
        for idx, file_path_ in enumerate(tqdm(self.data)):
            ...
            self.scan_points_rotated.append(torch.tensor(dd['points_posed_cloth'].transpose().transpose()).float())
            self.scan_normals_rotated.append(torch.tensor(dd['normals_posed_cloth'].transpose().transpose()).float()) # pymeshlab estimated normal is inverted
            self.scan_colors.append(torch.tensor(dd['colors_posed_cloth'].transpose().transpose()).float())
            self.scan_points.append(torch.tensor(scan_inv.transpose()).float())
            self.scan_normals.append(torch.tensor(normal_inv.transpose()).float())
    ...
    def _init_dataset(self):
        ...
        for idx, file_path_ in enumerate(tqdm(self.data)):
            ...
            if self.cano_available and not self.nsf_cano_available:
                ...
                cano_points = dd_cano['cano_points']
                cano_normals = dd_cano['cano_normals']
                valid_mask = dd_cano['valid_mask']

                num_points_input = cano_points.shape[0]
                if self.num_points >= 0:
                    idx_list = self._get_repeated_idx(num_points_input, self.num_points)
                else:
                    idx_list = np.arange(0, num_points_input)

                self.cano_points.append(torch.tensor(cano_points[idx_list].transpose()).float())
                self.cano_normals.append(torch.tensor(cano_normals[idx_list].transpose()).float())
                assert(dd['points_posed_cloth'].transpose()[valid_mask].shape==dd_cano['cano_points'].shape)
            ...
            if self.nsf_cano_available:
                ...
                self.cano_points.append(torch.tensor(cano_points[idx_list].transpose()).float())
                self.cano_normals.append(torch.tensor(cano_normals[idx_list].transpose()).float())
                self.coarse_cano_points.append(torch.tensor(coarse_cano_points[idx_list].transpose()).float())
                assert(dd['points_posed_cloth'].transpose()[valid_mask].shape==dd_cano['coarse_cano_points'].shape)

            # unrotate the additional root rotation -> no need to do in training
            r_rot_y = ScipyRot.from_rotvec(dd['rot_vector'])
            scan_inv = r_rot_y.inv().apply(dd['points_posed_cloth'].transpose()[valid_mask])
            normal_inv = r_rot_y.inv().apply(dd['normals_posed_cloth'].transpose()[valid_mask]) # pymeshlab estimated normal is inverted
            
            self.scan_points_rotated.append(torch.tensor(dd['points_posed_cloth'].transpose()[valid_mask][idx_list].transpose()).float())
            self.scan_normals_rotated.append(torch.tensor(dd['normals_posed_cloth'].transpose()[valid_mask][idx_list].transpose()).float()) # pymeshlab estimated normal is inverted
            self.scan_colors.append(torch.tensor(dd['colors_posed_cloth'].transpose()[valid_mask][idx_list].transpose()).float())
