...
class DataLoader_Cape_depth(BaseLoader):
    ...
    def __init__(self, mode, split_file, batch_size=64, num_workers=12, subject_index_dict=None, num_points=30000):  
        ...
        self.scan_points_rotated, self.scan_normals_rotated = [], []
        self.scan_points, self.scan_normals = [], []
        self.cano_cloth_points, self.cano_cloth_normals = [], []
        self.ref_points, self.ref_shaped_points = [], []
    ...
    def _init_dataset(self):
        ...
        for idx, file_path in enumerate(tqdm(self.data)):
            ...
            subject = split(file_path)[0].split('/')[12][:5]                     # A bit hardcoding in path, please adjust. 
            ...
            normal_inv = r_rot_y.inv().apply(dd['normals_posed_cloth'].transpose())
            
            self.scan_points_rotated.append(torch.tensor(dd['points_posed_cloth'].transpose()[idx_list].transpose()).float())
            self.scan_normals_rotated.append(torch.tensor(dd['normals_posed_cloth'].transpose()[idx_list].transpose()).float())
